...
{
   return video_Get("SELECT video.id, video.name, video.name_cn, video.link_video, video.highlights FROM  video ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function video_insert($obj)
{
    return exe_query("insert into video (name,name_cn,link_video,highlights) values ('$obj->name','$obj->name_cn','$obj->link_video','$obj->highlights')",'video');
}
//
function video_update($obj)
{
    return exe_query("update video set name='$obj->name',name_cn='$obj->name_cn',link_video='$obj->link_video',highlights='$obj->highlights' where id=$obj->id",'video');
}
//
