...
{
   return news_Get("SELECT news.id, danhmuc_tintuc.name as danhmuc_id, news.name, news.name_cn, news.name_url, news.img, news.content, news.content_cn, news.title, news.title_cn, news.keyword, news.keyword_cn, news.description, news.description_cn, news.created FROM  news, danhmuc_tintuc where danhmuc_tintuc.id=news.danhmuc_id  ".(($where!='')?(' and '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function news_insert($obj)
{
    return exe_query("insert into news (danhmuc_id,name,name_cn,name_url,img,content,content_cn,title,title_cn,keyword,keyword_cn,description,description_cn,created) values ('$obj->danhmuc_id','$obj->name','$obj->name_cn','$obj->name_url','$obj->img','$obj->content','$obj->content_cn','$obj->title','$obj->title_cn','$obj->keyword','$obj->keyword_cn','$obj->description','$obj->description_cn','$obj->created')",'news');
}
//
function news_update($obj)
{
    return exe_query("update news set danhmuc_id='$obj->danhmuc_id',name='$obj->name',name_cn='$obj->name_cn',name_url='$obj->name_url',img='$obj->img',content='$obj->content',content_cn='$obj->content_cn',title='$obj->title',title_cn='$obj->title_cn',keyword='$obj->keyword',keyword_cn='$obj->keyword_cn',description='$obj->description',description_cn='$obj->description_cn',created='$obj->created' where id=$obj->id",'news');
}
//
