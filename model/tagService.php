...
{
   return tag_Get("SELECT tag.id, tag.name, tag.name_cn, tag.link FROM  tag ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function tag_insert($obj)
{
    return exe_query("insert into tag (name,name_cn,link) values ('$obj->name','$obj->name_cn','$obj->link')",'tag');
}
//
function tag_update($obj)
{
    return exe_query("update tag set name='$obj->name',name_cn='$obj->name_cn',link='$obj->link' where id=$obj->id",'tag');
}
//
