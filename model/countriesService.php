...
{
   return countries_Get("SELECT countries.id, countries.country_code, countries.country_name FROM  countries ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function countries_insert($obj)
{
    return exe_query("insert into countries (country_code,country_name) values ('$obj->country_code','$obj->country_name')",'countries');
}
//
function countries_update($obj)
{
    return exe_query("update countries set country_code='$obj->country_code',country_name='$obj->country_name' where id=$obj->id",'countries');
}
//
